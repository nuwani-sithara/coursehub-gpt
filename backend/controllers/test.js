...
async function getHuggingFaceRecommendations(prompt, coursesContext, maxCourses) {
  ...
  console.log('Hugging Face raw response:', responseText);
  ...
  try {
    ...
    // If JSON parsing fails, try to extract course IDs manually
    const courseIds = [];
    const idRegex = /"courseId":\s*"([^"]+)"/g;
    let match;
    
    while ((match = idRegex.exec(responseText)) !== null) {
      courseIds.push({ courseId: match[1], reason: "Recommended based on your query" });
    }
    
    if (courseIds.length > 0) {
      return courseIds.slice(0, maxCourses);
    }
